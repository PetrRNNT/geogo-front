<template>
  <div class="places-layout">
    <div class="places-header">
      <h2 class="title">Все места <span class="count" v-if="places">{{ places.length }}</span></h2>
      <input v-model="filter" class="input search-input" placeholder="Фильтр по названию или городу..." />
    </div>
...
export default {
  data() {
    return { places: [], loading: true, error: null, filter: '', selected: null }
  },
  computed: {
    filtered() {
...
<style scoped>
.places-layout { display: flex; flex-direction: column; gap: 20px; }
.places-header { display: flex; align-items: center; gap: 16px; flex-wrap: wrap; }
...
.state-msg { color: var(--text-muted); font-size: 15px; padding: 40px 0; text-align: center; }
...
.place-card:hover { border-color: #2d4a6e; transform: translateY(-1px); }
.place-card.selected { border-color: var(--accent); }
...
.place-country { font-size: 12px; color: var(--text-muted); white-space: nowrap; }
.place-address { font-size: 13px; color: var(--text-muted); }
.place-coords { font-family: var(--font-mono); font-size: 12px; color: var(--accent); margin-top: 4px; }

.detail-panel {
  position: fixed; right: 24px; bottom: 24px;
  background: var(--bg-card); border: 1px solid var(--accent);
  border-radius: 12px; width: 300px; box-shadow: 0 8px 32px rgba(0,0,0,0.4);
}
.detail-inner { padding: 20px; display: flex; flex-direction: column; gap: 10px; position: relative; }
.close-btn {
  position: absolute; top: 14px; right: 14px;
  background: none; border: none; color: var(--text-muted); cursor: pointer; font-size: 14px;
}
.close-btn:hover { color: var(--text); }
.detail-name { font-size: 18px; font-weight: 600; padding-right: 24px; }
.detail-row { display: flex; gap: 8px; font-size: 14px; }
.detail-label { color: var(--text-muted); min-width: 60px; }
.detail-coords {
  font-family: var(--font-mono); font-size: 13px; color: var(--accent);
  background: var(--accent-dim); padding: 8px 12px; border-radius: 6px;
}
.coords-tag { font-size: 10px; opacity: 0.6; margin-right: 6px; }
.osm-link {
  color: var(--accent); font-size: 13px; text-decoration: none;
  padding-top: 4px; transition: opacity 0.15s;
}
.osm-link:hover { opacity: 0.75; }
</style>
